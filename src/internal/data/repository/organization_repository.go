...
func (receiver *OrganizationRepository) UserJoinOrganization(req request.UserJoinOrganizationRequest) error {
	...
	var count int64
	err = receiver.DBConn.Model(&entity.SUsersOrganization{}).
	...
	result := receiver.DBConn.Create(&entity.SUsersOrganization{
		UserId:         user.ID,
...
func (receiver *OrganizationRepository) GetAllUserByOrganization(organizationID uint) ([]*entity.SUserEntity, error) {
	...
	err := receiver.DBConn.Raw("SELECT * FROM s_user_entity sue WHERE sue.id IN (SELECT suo.user_id FROM s_users_organization suo WHERE organization_id = ?)", organizationID).
		Scan(&users).Error
